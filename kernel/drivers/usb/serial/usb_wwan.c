...
static void dump_data(struct usb_interface *interface,
    ...
    printk(KERN_DEBUG "[usb_wwan]: dump %s(if#%d, len %d), chars = \"",
            str, n, size);
    for (i = 0; i < size; ++i) {
        char c = data[i];
        if (c > 0x20 && c < 0x7e) {
...
static void usb_wwan_indat_callback(struct urb *urb)
{
	...
	if (status) {
		...
		dump_data(port->serial->interface, __func__, data, urb->actual_length);
...
static void usb_wwan_outdat_callback(struct urb *urb)
{
	...
	if (urb->status < 0)
		printk(KERN_ERR "[usb_wwan]: %s fail to send data to cbp %d\n", __func__, urb->status);
	else
		dump_data(port->serial->interface, __func__, urb->transfer_buffer, urb->transfer_buffer_length);
