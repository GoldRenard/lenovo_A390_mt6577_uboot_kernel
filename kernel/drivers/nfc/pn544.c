		...
		size_t count, loff_t *offset)
{
	...
	if (ret != count) {
		my_pr_err("%s : i2c_master_send returned %d\n", __func__, ret);
		ret = -EIO;
	}
			    ...
			    unsigned int cmd, unsigned long arg)
{
	...
	case PN544_SET_PWR:
		if (arg == 2) {
			 ...
			 */
			my_pr_info("%s power on with firmware\n", __func__);
			...
			bool fgPowerStatus;
			/* power on */
			my_pr_info("%s power on, firm_gpio: %d, ven_gpio: %d !!!\n", __func__, pn544_dev->firm_gpio, pn544_dev->ven_gpio);
			//gpio_set_value(pn544_dev->firm_gpio, 0);
			//gpio_set_value(pn544_dev->ven_gpio, 1);

      //LiangChi,20110617    
      //fgPowerStatus = hwPowerOn(MT65XX_POWER_LDO_VCAMD2, VOL_1800, "VCAMD2");   
      my_pr_info("%s,power_on CAMERA_POWER_VCAM[%d]", __func__, fgPowerStatus);

			my_pr_info("%s [firm_gpio:%d](1) mode=%d, dir=%d, old=%d, new=%d", __func__,
			   pn544_dev->firm_gpio,
			...
			my_pr_info("%s [firm_gpio:%d](2) mode=%d, dir=%d, current=%d", __func__,
			   pn544_dev->firm_gpio,
            mt_get_gpio_mode(pn544_dev->firm_gpio),
            mt_get_gpio_dir(pn544_dev->firm_gpio),
            mt_get_gpio_out(pn544_dev->firm_gpio));
			...
			my_pr_info("%s [ven_gpio:%d](1) mode=%d, dir=%d, old=%d, new=%d", __func__,
			   pn544_dev->ven_gpio,
            ...
            #if 0
            #warning testing code. NEED to be removed afterward.
			mt_set_gpio_out(pn544_dev->ven_gpio, GPIO_OUT_ONE);
			msleep(10);
			mt_set_gpio_out(pn544_dev->ven_gpio, GPIO_OUT_ZERO);
			msleep(50);
			...
			mt_set_gpio_out(pn544_dev->ven_gpio, GPIO_OUT_ONE);
			#endif
			my_pr_info("%s [ven_gpio:%d](2) mode=%d, dir=%d, current=%d", __func__,
			   pn544_dev->ven_gpio,
            mt_get_gpio_mode(pn544_dev->ven_gpio),
            mt_get_gpio_dir(pn544_dev->ven_gpio),
            mt_get_gpio_out(pn544_dev->ven_gpio));

//			my_pr_info("%s enable irq", __func__);
//      	mt65xx_eint_unmask(CUST_EINT_NFC_NUM);
			...
			bool fgPowerStatus;
			/* power off */
			my_pr_info("%s power off, firm_gpio: %d, ven_gpio: %d\n", __func__, pn544_dev->firm_gpio, pn544_dev->ven_gpio);
			//gpio_set_value(pn544_dev->firm_gpio, 0);
			//gpio_set_value(pn544_dev->ven_gpio, 0);

			my_pr_info("%s [firm_gpio:%d](3) mode=%d, dir=%d, old=%d, new=%d", __func__,
			   pn544_dev->firm_gpio,
            mt_get_gpio_mode(pn544_dev->firm_gpio),
            mt_get_gpio_dir(pn544_dev->firm_gpio),
            mt_get_gpio_out(pn544_dev->firm_gpio),
			   GPIO_OUT_ZERO);
			mt_set_gpio_out(pn544_dev->firm_gpio, GPIO_OUT_ZERO);
			my_pr_info("%s [firm_gpio:%d](4) mode=%d, dir=%d, current=%d", __func__,
			   pn544_dev->firm_gpio,
            mt_get_gpio_mode(pn544_dev->firm_gpio),
            mt_get_gpio_dir(pn544_dev->firm_gpio),
            mt_get_gpio_out(pn544_dev->firm_gpio));

			my_pr_info("%s [ven_gpio:%d](3) mode=%d, dir=%d, old=%d, new=%d", __func__,
			   pn544_dev->ven_gpio,
            mt_get_gpio_mode(pn544_dev->ven_gpio),
            mt_get_gpio_dir(pn544_dev->ven_gpio),
            mt_get_gpio_out(pn544_dev->ven_gpio),
			...
			mt_set_gpio_out(pn544_dev->ven_gpio, GPIO_OUT_ZERO);
			#endif
			my_pr_info("%s [ven_gpio:%d](4) mode=%d, dir=%d, current=%d", __func__,
			   pn544_dev->ven_gpio,
            mt_get_gpio_mode(pn544_dev->ven_gpio),
            mt_get_gpio_dir(pn544_dev->ven_gpio),
            mt_get_gpio_out(pn544_dev->ven_gpio));
      ...
      my_pr_info("%s,power_off CAMERA_POWER_VCAM[%d]", __func__, fgPowerStatus);
      
		} else {
			my_pr_err("%s bad arg %lu\n", __func__, arg);
			return -EINVAL;
		}
		break;
	default:
