...
static int  adxl346_local_init(void);
static int  adxl346_remove(void);
static int adxl346_init_flag =0; // 0<==>OK -1 <==> fail
/*----------------------------------------------------------------------------*/
typedef enum {
    ADX_TRC_FILTER  = 0x01,
...
/*----------------------------------------------------------------------------*/
static struct sensor_init_info adxl346_init_info = {
		.name = "adxl346",
		.init = adxl346_local_init,
		.uninit = adxl346_remove,
};
...
/*----------------------------------------------------------------------------*/
int adxl346_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static int adxl346_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = adxl346_get_cust_acc_hw();
	...
	if((err = adxl346_create_attr(&(adxl346_init_info.platform_diver_addr->driver))))
	{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = adxl346_operate;
...
static int adxl346_i2c_remove(struct i2c_client *client)
{
	...
	if((err = adxl346_delete_attr(&(adxl346_init_info.platform_diver_addr->driver))))
	{
...
static int  adxl346_remove(void)
{
    struct acc_hw *hw = adxl346_get_cust_acc_hw();
...
static int __init adxl346_init(void)
{
	...
	hwmsen_gsensor_add(&adxl346_init_info);
#if 0	
