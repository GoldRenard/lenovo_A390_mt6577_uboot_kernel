...
{             
    ...
    *(volatile u16 *) TPD_RESET_PIN_ADDR = temp;
    ...
    printk("MediaTek touch panel i2c probe - GT811 \n");
...
#if 1
    memset( &version[0], 0, 2);
    err = i2c_read_bytes( i2c_client, TPD_VERSION_INFO_REG, (u8 *)&version[0], 2);
    ...
    {
        printk( "Goodix TouchScreen Version:0x%x%x\n", version[0],version[1]);
    }
#endif    
...
{   
	...
	err = i2c_read_bytes( i2c_client, TPD_CONFIG_REG_BASE, &buf106[0], 106);
	...
	if ( isAttacked )
	{
	    printk( "Touch panel is attacked!! Start to initialize panel ..\n");
		tpd_reset();
	}
	else
	{
...
int gt811_open ( struct inode *inode, struct file *filp )
{
	TPD_DEBUG("gt811 open!\n");
	return 0;
}

int gt811_close ( struct inode *inode, struct file *filp )
{
	TPD_DEBUG("gt811 close!\n");
	return 0;
}

static long gt811_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	TPD_DEBUG("gt811 ioctl!\n");

	switch ( cmd )
	{
		case TPD_IOCTL_ENABLE_I2C:
			TPD_DEBUG("gt811 TPD_IOCTL_ENABLE_I2C!\n");
			mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
			break;
			
		case TPD_IOCTL_DISABLE_I2C:
			TPD_DEBUG("gt811 TPD_IOCTL_DISABLE_I2C!\n");
			mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
			break;
			
		default:
			TPD_DEBUG("gt811 command does not exist!\n");
			break;
	}
...
struct file_operations gt811_fops = {
	.owner = THIS_MODULE,
	.open = gt811_open,
	.release = gt811_close,
	.unlocked_ioctl = gt811_unlocked_ioctl,
};
...
void tpd_key_down ( u8 keyStatus )
{
	...
	else if ( TPD_KEY_HOME == keyStatus )
    {
		...
		TPD_DEBUG("[mtk-tpd] Press key homepage !\n");
    }
	else if ( TPD_KEY_BACK== keyStatus )
    {
        input_report_key( tpd->dev, KEY_BACK, 1 );
		TPD_DEBUG("[mtk-tpd] Press key back !\n");
    }
	else if ( TPD_KEY_SEARCH== keyStatus )
    {
        input_report_key( tpd->dev, KEY_SEARCH, 1 );
		TPD_DEBUG("[mtk-tpd] Press key SEARCH !\n");
    }
...
//	u8 touchBufferStatus;//keyStatus lastKey = 0x00;
    ...
    struct work_struct evtwork;
	
      memset( (void *)&evtwork, 0, sizeof(evtwork));
  ...
  sched_setscheduler(current, SCHED_RR, &param); 
    do{
		...
		touchSensorId= buffer[0] & 0xC0;
//		if ( 0x80 != touchBufferStatus )
//		{
//			TPD_DEBUG("[mtk-tpd] Touch Sensor ID : %x \n", touchSensorId);
...
//		TPD_DEBUG("[mtk-tpd] STATUS : %x \n", cur_mask);		
		TPD_DEBUG("[mtk-tpd] TouchPointFlag(0x721) : %x \n", buffer[0]);		
		...
		TPD_DEBUG("[mtk-tpd] Touch KEY STATUS(0x722) : 0x%x \n", keyStatus);

		if ( 0x0F == keyStatus )
		{
    			printk("[mtk-tpd] KEY Status is 0xF , we apply esd check!\n");
			tpd_esd_check_func(&evtwork);
			continue;
		}
...
int tpd_local_init(void) 
{
	...
	/* register device (/dev/GT811 CTP) */
	retval = misc_register( &tpd_dev );
	TPD_DEBUG("GT811 call misc_register\n");
	if ( 0 != retval )
	{
		TPD_DEBUG("Register gt811 device failed (%d)\n", retval);
		return -1;
	}
...
static void tpd_suspend( struct early_suspend *h )
{
    ...
    tpd_halt = 1;
    mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
    TPD_DEBUG("GT811 call early suspend\n");
...
static void tpd_resume( struct early_suspend *h )
{
    ...
    TPD_DEBUG("GT811 call early resume\n");
...
{
    printk("MediaTek gt811 touch panel driver init\n");
...
{
    TPD_DMESG("MediaTek gt811 touch panel driver exit\n");
