 ...
 *
 * 02 19 2012 koli.lin
 * [ALPS00237768] [A52][??]¦Z¸mcamera a5142 AF?µJºC
...
MSDK_SCENARIO_ID_ENUM A5142_CurrentScenarioId = ACDK_SCENARIO_ID_CAMERA_PREVIEW;
...
static kal_uint16 A5142gain2reg(kal_uint16 gain)
{
	...
	// a5142 min reg gain 0x112E , mtk gain 122, so here block litter than min gian value
	/*	
...
static void A5142MIPI_Init_setting(void)
{
	...
	SENSORDB( "A5142MIPI initial setting \n");
	...
	A5142MIPI_write_cmos_sensor_8(0x0100, 0x00);	// MODE_SELECT
	    ...
	    do
	    {
			...
			mDELAY(2);	
			status = A5142MIPI_read_cmos_sensor(0x301A);    //polling status change 
			SENSORDB("A5142MIPI status = %x \n",status);
		}while(status != 0x12C8)   ;
	...
	A5142MIPI_write_cmos_sensor(0x30F0, 0x0000);  	// disable AF  A5142 have not AF function
...
kal_uint16 A5142MIPI_PowerOn(void)
{
	const kal_uint8 i2c_addr[] = {A5142MIPI_WRITE_ID_1, A5142MIPI_WRITE_ID_2, 0x6C, 0x6E}; 
...
UINT32 A5142MIPIOpen(void)
{
	...
	sensor_id = A5142MIPI_PowerOn() ;
	
	SENSORDB("A5142MIPIOpen sensor_id is %x \n", sensor_id);
    if (sensor_id != A5142MIPI_SENSOR_ID)
...
{
	*sensorID  = A5142MIPI_PowerOn() ;
		
	SENSORDB("A5142MIPIGetSensorID sensor_id is %x\n", *sensorID );
...
static void A5142MIPI_capture_setting(void)
{
	...
	//stop_streaming
	//A5142MIPI_write_cmos_sensor_8(0x0100, 0x00); 	// MODE_SELECT stop streaming
	#ifdef MIPI_INTERFACE
		#ifdef RAW10
			//A5142MIPI_write_cmos_sensor(0x301A, 0x0018);	// enable mipi interface
			A5142MIPI_write_cmos_sensor(0x3064, 0xB800);	// SMIA_TEST
	...
	A5142MIPI_write_cmos_sensor_8(0x0104, 0x00); //Grouped Parameter Hold = 0x0
	
	A5142MIPI_write_cmos_sensor_8(0x0100, 0x01);	//mode_select (Open streaming)
...
static void A5142MIPI_SetDummy(kal_bool mode,const kal_uint16 iDummyPixels, const kal_uint16 iDummyLines)
{
	...
	{
		Line_length_pclk   = A5142MIPI_PV_PERIOD_PIXEL_NUMS + iDummyPixels;
		Frame_length_lines = A5142MIPI_PV_PERIOD_LINE_NUMS  + iDummyLines;
	}
	else   //capture
	{
		Line_length_pclk   = A5142MIPI_FULL_PERIOD_PIXEL_NUMS + iDummyPixels;
		Frame_length_lines = A5142MIPI_FULL_PERIOD_LINE_NUMS  + iDummyLines;
	}
    
    spin_lock(&a5142mipiraw_drv_lock);
    A5142_Frame_Length_preview = Frame_length_lines;
    spin_unlock(&a5142mipiraw_drv_lock);
	
    SENSORDB("Enter A5142MIPI_SetDummy Frame_length_lines=%d, Line_length_pclk=%d\n",Frame_length_lines,Line_length_pclk);

	A5142MIPI_write_cmos_sensor_8(0x0104, 0x01);// GROUPED_PARAMETER_HOLD
	A5142MIPI_write_cmos_sensor(0x0340, Frame_length_lines);
	A5142MIPI_write_cmos_sensor(0x0342, Line_length_pclk);
	A5142MIPI_write_cmos_sensor_8(0x0104, 0x00); //Grouped Parameter Hold = 0x00
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    image_window->GrabStartX= A5142MIPI_PV_START_X;
    image_window->GrabStartY= A5142MIPI_PV_START_Y;
    image_window->ExposureWindowWidth= A5142MIPI_IMAGE_SENSOR_PV_WIDTH;
    image_window->ExposureWindowHeight= A5142MIPI_IMAGE_SENSOR_PV_HEIGHT;
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if ((image_window->ImageTargetWidth<= A5142MIPI_IMAGE_SENSOR_PV_WIDTH) &&
		  (image_window->ImageTargetHeight<= A5142MIPI_IMAGE_SENSOR_PV_HEIGHT))
	{	
		SENSORDB("Enter A5142MIPICapture small size  \n ");

		spin_lock(&a5142mipiraw_drv_lock);
		A5142MIPI_sensor.capture_vt_clk = A5142MIPI_sensor.preview_vt_clk;
		spin_unlock(&a5142mipiraw_drv_lock);
		...
		spin_unlock(&a5142mipiraw_drv_lock);
		A5142MIPI_SetDummy(KAL_TRUE, A5142MIPI_dummy_pixels, A5142MIPI_dummy_lines);
		
		shutter = shutter * (A5142MIPI_PV_PERIOD_PIXEL_NUMS + A5142MIPI_PV_dummy_pixels)/(A5142MIPI_PV_PERIOD_PIXEL_NUMS +A5142MIPI_dummy_pixels);
	...
	{
		spin_lock(&a5142mipiraw_drv_lock);
		if(g_iA5142MIPI_Mode == A5142MIPI_MODE_CAPTURE) {
			spin_unlock(&a5142mipiraw_drv_lock);
	       	return;
		}
		...
		SENSORDB("Enter A5142MIPICapture full size  \n ");
		...
		image_window->GrabStartX           = A5142MIPI_FULL_START_X;
        image_window->GrabStartY           = A5142MIPI_FULL_START_Y;
        image_window->ExposureWindowWidth  = A5142MIPI_IMAGE_SENSOR_FULL_WIDTH;
        image_window->ExposureWindowHeight = A5142MIPI_IMAGE_SENSOR_FULL_HEIGHT;
	}
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    switch (ScenarioId)
    {
        ...
        default:
            ...
            pSensorInfo->SensorDataLatchCount= 2;
            pSensorInfo->SensorGrabStartX = A5142MIPI_PV_START_X; 
            pSensorInfo->SensorGrabStartY = A5142MIPI_PV_START_X;             
            break;
    }

   // A5142MIPIPixelClockDivider=pSensorInfo->SensorPixelClockCount;
...
UINT32 A5142MIPISetVideoMode(UINT16 u2FrameRate)
{
	...
	SENSORDB("A5142MIPISetVideoMode u2FrameRate =%d",u2FrameRate);
...
{
    SENSORDB("[A5142MIPISetAutoFlickerMode] frame rate(10base) = %d %d\n", bEnable, u2FrameRate);
    if(bEnable) 
	{   
		// enable auto flicker   
