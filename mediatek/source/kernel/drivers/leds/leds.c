...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK3)
	{
		...
		if(first_time == true)
		{
			hwBacklightISINKTurnOff(3); //sw workround for sync leds status 
			first_time = false;
		}
	        ...
	        {
		        hwBacklightISINKTuning(3, PMIC_PWM_1, 0x0, 0);
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK4)
	{
		if(first_time == true)
		{
			hwBacklightISINKTurnOff(5);  //sw workround for sync leds status 
			first_time = false;
		}
		...
		//if(led_init_flag[0] == false)
		{
			hwBacklightISINKTuning(4, PMIC_PWM_1, 0x0, 0);
			hwPWMsetting(PMIC_PWM_1, 15, 8);
			led_init_flag[0] = true;
		}
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK5)
	{
		if(first_time == true)
		{
			hwBacklightISINKTurnOff(4);  //sw workround for sync leds status
			first_time = false;
		}
		...
		{
			hwBacklightISINKTuning(5, PMIC_PWM_2, 0x0, 0);
